...
Reactivity Transform is currently an experimental feature. It is disabled by default and requires [[vue3.#explicit-opt-in]]. It may also change before being finalized. To stay up-to-date, keep an eye on its [proposal and discussion on GitHub](https://github.com/vuejs/rfcs/discussions/369).
:::
...
- [[vue3.api.reactivity-core#ref]] -> `$ref`
- [[vue3.api.reactivity-core#computed]] -> `$computed`
- [[vue3.api.reactivity-advanced#shallowref]] -> `$shallowRef`
- [[vue3.api.reactivity-advanced#customref]] -> `$customRef`
- [[vue3.api.reactivity-utilities#toref]] -> `$toRef`
...
2. When using the [[vue3.api.sfc-script-setup#typescript-only-features]], there is no easy way to declare default values for the props. We introduced the `withDefaults()` API for this exact purpose, but it's still clunky to use.
