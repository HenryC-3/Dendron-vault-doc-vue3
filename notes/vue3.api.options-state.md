...
- **See also:** [[vue3.guide.extras.reactivity-in-depth]]
...
- **Details**
  ...
  - **`type`**: Can be one of the following native constructors: `String`, `Number`, `Boolean`, `Array`, `Object`, `Date`, `Function`, `Symbol`, any custom constructor function or an array of those. In development mode, Vue will check if a prop's value matches the declared type, and will throw a warning if it doesn't. See [[vue3.guide.components.props#prop-validation]] for more details.

    Also note that a prop with `Boolean` type affects its value casting behavior in both development and production. See [[vue3.guide.components.props#boolean-casting]] for more details.
...
- **See also:** [[vue3.guide.components.props]]
...
- **See also:** [[vue3.guide.essentials.computed]]
...
- **See also:** [[vue3.guide.essentials.event-handling]]
...
- **Details**
  ...
  In addition to a root-level property, the key can also be a simple dot-delimited path, e.g. `a.b.c`. Note that this usage does **not** support complex expressions - only dot-delimited paths are supported. If you need to watch complex data sources, use the imperative [[vue3.api.component-instance#watch]] API instead.
  ...
  - **`deep`**: force deep traversal of the source if it is an object or an array, so that the callback fires on deep mutations. See [[vue3.guide.essentials.watchers#deep-watchers]].
...
- **See also:** [[vue3.guide.essentials.watchers]]
...
- **Details**
  ...
  Note that the `emits` option affects which event listeners are considered component event listeners, rather than native DOM event listeners. The listeners for declared events will be removed from the component's `$attrs` object, so they will not be passed through to the component's root element. See [[vue3.guide.components.attrs]] for more details.
