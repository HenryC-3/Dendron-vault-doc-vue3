...
- [[vue3.api.reactivity-core]], e.g. `ref()` and `reactive()`, that allows us to directly create reactive state, computed state, and watchers.

- [[vue3.api.composition-api-lifecycle]], e.g. `onMounted()` and `onUnmounted()`, that allow us to programmatically hook into the component lifecycle.

- [[vue3.api.composition-api-dependency-injection]], i.e. `provide()` and `inject()`, that allow us to leverage Vue's dependency injection system while using Reactivity APIs.

Composition API is a built-in feature of Vue 3, and is currently available to Vue 2 via the officially maintained [`@vue/composition-api`](https://github.com/vuejs/composition-api) plugin. In Vue 3, it is also primarily used together with the [[vue3.api.sfc-script-setup]] syntax in Single-File Components. Here's a basic example of a component using Composition API:
...
The primary advantage of Composition API is that it enables clean, efficient logic reuse in the form of [[vue3.guide.reusability.composables]]. It solves [[vue3.guide.reusability.composables#vs-mixins]], the primary logic reuse mechanism for Options API.

Composition API's logic reuse capability has given rise to impressive community projects such as [VueUse](https://vueuse.org/), an ever-growing collection of composable utilities. It also serves as a clean mechanism for easily integrating stateful third-party services or libraries into Vue's reactivity system, for example [[vue3.guide.extras.reactivity-in-depth#immutable-data]], [[vue3.guide.extras.reactivity-in-depth#state-machines]], and [RxJS](https://vueuse.org/rxjs/readme.html#vueuse-rxjs).
...
![folder component after](/assets/composition-api-after-f8bdypgxfws1.png)
...
Yes. You can use Composition API via the [[vue3.api.composition-api-setup]] option in an Options API component.
