...
With the Options API, we can use the [[vue3.api.options-state#watch]] to trigger a function whenever a reactive property changes:
...
With Composition API, we can use the [[vue3.api.reactivity-core#watch]] to trigger a callback whenever a piece of reactive state changes:
...
This can be simplified with [[vue3.api.reactivity-core#watcheffect]]. `watchEffect()` allows us to perform a side effect immediately while automatically tracking the effect's reactive dependencies. The above example can be rewritten as:
...
You can check out [[vue3.examples.#fetching-data]] with `watchEffect` and reactive data-fetching in action.
...
It's also possible to imperatively create watchers using the [[vue3.api.component-instance#watch]]:
