...
If you are familiar with Virtual DOM concepts and prefer the raw power of JavaScript, you can also [[vue3.guide.extras.render-function]] instead of templates, with optional JSX support. However, do note that they do not enjoy the same level of compile-time optimizations as templates.
...
The double mustaches interpret the data as plain text, not HTML. In order to output real HTML, you will need to use the [[vue3.api.built-in-directives#v-html]]:
...
Mustaches cannot be used inside HTML attributes. Instead, use a [[vue3.api.built-in-directives#v-bind]]:
...
Globals not explicitly included in the list, for example user-attached properties on `window`, will not be accessible in template expressions. You can, however, explicitly define additional globals for all Vue expressions by adding them to [[vue3.api.application#app-config-globalproperties]].

## Directives

Directives are special attributes with the `v-` prefix. Vue provides a number of [[vue3.api.built-in-directives]], including `v-html` and `v-bind` which we have introduced above.

Directive attribute values are expected to be single JavaScript expressions (with the exception of `v-for`, `v-on` and `v-slot`, which will be discussed in their respective sections later). A directive's job is to reactively apply updates to the DOM when the value of its expression changes. Take [[vue3.api.built-in-directives#v-if]] as an example:
...
If you need to pass a complex dynamic argument, it's probably better to use a [[vue3.computed]], which we will cover shortly.
...
You'll see other examples of modifiers later, [[vue3.event-handling#event-modifiers]] and [[vue3.forms#modifiers]], when we explore those features.

And finally, here's the full directive syntax visualized:

![directive syntax graph](/assets/directive-xthygvdwkoay.png)
