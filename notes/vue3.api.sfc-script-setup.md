...
Reactive state needs to be explicitly created using [[vue3.reactivity-core]]. Similar to values returned from a `setup()` function, refs are automatically unwrapped when referenced in templates:
...
If you are using TypeScript, it is also possible to [[vue3.#typescript-only-features]].
