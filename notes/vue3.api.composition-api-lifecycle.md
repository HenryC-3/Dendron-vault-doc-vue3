...
:::info Usage Note
All APIs listed on this page must be called synchronously during the `setup()` phase of a component. See [[vue3.guide.essentials.lifecycle]] for more details.
:::
...
- **Details**
  ...
  This hook is typically used for performing side effects that need access to the component's rendered DOM, or for limiting DOM-related code to the client in a [[vue3.guide.scaling-up.ssr]].
...
- **Details**
  ...
  This hook is called after any DOM update of the component, which can be caused by different state changes. If you need to access the updated DOM after a specific state change, use [[vue3.api.general#nexttick]] instead.
...
- **Details**
  ...
  **Error Propagation Rules**
  ...
  - By default, all errors are still sent to the application-level [[vue3.api.application#app-config-errorhandler]] if it is defined, so that these errors can still be reported to an analytics service in a single place.
...
Registers a callback to be called after the component instance is inserted into the DOM as part of a tree cached by [[vue3.api.built-in-components#keepalive]].
...
- **See also:** [[vue3.guide.built-ins.keep-alive#lifecycle-of-cached-instance]]

## onDeactivated()

Registers a callback to be called after the component instance is removed from the DOM as part of a tree cached by [[vue3.api.built-in-components#keepalive]].
...
- **See also:** [[vue3.guide.scaling-up.ssr]]
