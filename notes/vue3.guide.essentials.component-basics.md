...
![Component Tree](/assets/components-j3zlnt9fov9m.png)

<!-- https://www.figma.com/file/qa7WHDQRWuEZNRs7iZRZSI/components -->

This is very similar to how we nest native HTML elements, but Vue implements its own component model that allow us to encapsulate custom content and logic in each component. Vue also plays nicely with native Web Components. If you are curious about the relationship between Vue Components and native Web Components, [[vue3.guide.extras.web-components]].

## Defining a Component

When using a build step, we typically define each Vue component in a dedicated file using the `.vue` extension - known as a [[vue3.guide.scaling-up.sfc]] (SFC for short):
...
:::tip
We will be using SFC syntax for the rest of this guide - the concepts around components are the same regardless of whether you are using a build step or not. The [[vue3.examples.]] section shows component usage in both scenarios.
:::
...
To expose the imported component to our template, we need to [[vue3.guide.components.registration]] it with the `components` option. The component will then be available as a tag using the key it is registered under.
...
It's also possible to globally register a component, making it available to all components in a given app without having to import it. The pros and cons of global vs. local registration is discussed in the dedicated [[vue3.guide.components.registration]] section.
...
See also: [[vue3.guide.typescript.composition-api#typing-component-props]] <sup class="vt-badge ts" />
...
That's all you need to know about props for now, but once you've finished reading this page and feel comfortable with its content, we recommend coming back later to read the full guide on [[vue3.guide.components.props]].
...
Then the child component can emit an event on itself by calling the built-in [[vue3.api.component-instance#emit]], passing the name of the event:
...
We can optionally declare emitted events using the <span class="options-api">[[vue3.api.options-state#emits]] option</span><span class="composition-api">[[vue3.api.sfc-script-setup#defineprops-defineemits]] macro</span>:
...
This documents all the events that a component emits and optionally [[vue3.guide.components.events#events-validation]]. It also allows Vue to avoid implicitly applying them as native listeners to the child component's root element.
...
See also: [[vue3.guide.typescript.composition-api#typing-component-emits]] <sup class="vt-badge ts" />
...
That's all you need to know about custom component events for now, but once you've finished reading this page and feel comfortable with its content, we recommend coming back later to read the full guide on [[vue3.guide.components.events]].
...
That's all you need to know about slots for now, but once you've finished reading this page and feel comfortable with its content, we recommend coming back later to read the full guide on [[vue3.guide.components.slots]].
...
When switching between multiple components with `<component :is="...">`, a component will be unmounted when it is switched away from. We can force the inactive components to stay "alive" with the built-in [[vue3.guide.built-ins.keep-alive]].
...
The custom component `<blog-post-row>` will be hoisted out as invalid content, causing errors in the eventual rendered output. We can use the special [[vue3.api.built-in-special-attributes#is]] as a workaround:
