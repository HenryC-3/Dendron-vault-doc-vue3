...
- **Details**
  ...
  Only use template sources that you can trust. Do not use user-provided content as your template. See [[vue3.guide.best-practices.security#rule-no-1-never-use-non-trusted-templates]] for more details.
  :::
...
- **See also:**
  - [[vue3.guide.extras.rendering-mechanism]]
  - [[vue3.guide.extras.render-function]]
...
- **Details**
  ...
  This config option is only respected when using the full build (i.e. the standalone `vue.js` that can compile templates in the browser). It supports the same options as the app-level [[vue3.api.application#app-config-compileroptions]], and has higher priority for the current component.

- **See also:** [[vue3.api.application#app-config-compileroptions]]
