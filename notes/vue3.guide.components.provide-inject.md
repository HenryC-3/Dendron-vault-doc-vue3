...
Usually, when we need to pass data from the parent to a child component, we use [[vue3.guide.components.props]]. However, imagine the case where we have a large component tree, and a deeply nested component needs something from a distant ancestor component. With only props, we would have to pass the same prop across the entire parent chain:

![prop drilling diagram](/assets/prop-drilling-sq8br3jnbpoh.png)
...
![Provide/inject scheme](/assets/provide-inject-4fq4r1gu9a06.png)
...
To provide data to a component's descendants, use the [[vue3.api.composition-api-dependency-injection#provide]] function:
...
To provide data to a component's descendants, use the [[vue3.api.options-composition#provide]] option:
...
However, do note this does **not** make the injection reactive. We will discuss [[vue3.#working-with-reactivity]] below.
...
App-level provides are available to all components rendered in the app. This is especially useful when writing [[vue3.guide.reusability.plugins]], as plugins typically wouldn't be able to provide values using components.

## Inject

<div class="composition-api">

To inject data provided by an ancestor component, use the [[vue3.api.composition-api-dependency-injection#inject]] function:
...
To inject data provided by an ancestor component, use the [[vue3.api.options-composition#inject]] option:
...
Finally, you can wrap the provided value with [[vue3.api.reactivity-core#readonly]] if you want to ensure that the data passed through `provide` cannot be mutated by the injected component.
...
In order to make injections reactively linked to the provider, we need to provide a computed property using the [[vue3.api.reactivity-core#computed]] function:
...
The `computed()` function is typically used in Composition API components, but can also be used to complement certain use cases in Options API. You can learn more about its usage by reading the [[vue3.guide.essentials.reactivity-fundamentals]] and [[vue3.guide.essentials.computed]] with the API Preference set to Composition API.
...
See also: [[vue3.guide.typescript.composition-api#typing-provide-inject]] <sup class="vt-badge ts" />
