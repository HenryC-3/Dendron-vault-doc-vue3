...
Vue supports creating custom elements using exactly the same Vue component APIs via the [[vue3.api.general#definecustomelement]] method. The method accepts the same argument as [[vue3.api.general#definecomponent]], but instead returns a custom element constructor that extends `HTMLElement`:
...
- [[vue3.guide.components.slots#scoped-slots]] are not supported.
