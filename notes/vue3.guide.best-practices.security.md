...
When a vulnerability is reported, it immediately becomes our top concern, with a full-time contributor dropping everything to work on it. To report a vulnerability, please email [[vue3.mailto:security@vuejs.org]].
...
To keep your users fully safe from click jacking, we recommend only allowing full control over CSS inside a sandboxed iframe. Alternatively, when providing user control through a style binding, we recommend using its [[vue3.guide.essentials.class-and-style#binding-to-objects-1]] and only allowing users to provide values for specific properties it's safe for them to control, like this:
...
Beyond the recommendations made above for [[vue3.#potential-dangers]], we also recommend familiarizing yourself with these resources:
...
There are some additional security concerns when using SSR, so make sure to follow the best practices outlined throughout [[vue3.guide.scaling-up.ssr]] to avoid vulnerabilities.
