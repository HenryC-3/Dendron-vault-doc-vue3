...
:::warning Note
It's **not** recommended to use `v-if` and `v-for` on the same element due to implicit precedence. Refer to [[vue3.style-guide.rules-essential#avoid-v-if-with-v-for]] for details.
:::
...
:::tip Note
`key` here is a special attribute being bound with `v-bind`. It should not be confused with the property key variable when [[vue3.#v-for-with-an-object]].
:::

[[vue3.style-guide.rules-essential#use-keyed-v-for]] to provide a `key` attribute with `v-for` whenever possible, unless the iterated DOM content is simple (i.e. contains no components or stateful DOM elements), or you are intentionally relying on the default behavior for performance gains.

The `key` binding expects primitive values - i.e. strings and numbers. Do not use objects as `v-for` keys. For detailed usage of the `key` attribute, please see the [[vue3.api.built-in-special-attributes#key]].

## `v-for` with a Component

> This section assumes knowledge of [[vue3.guide.essentials.component-basics]]. Feel free to skip it and come back later.
