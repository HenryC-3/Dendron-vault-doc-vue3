...
By default, `v-model` syncs the input with the data after each `input` event (with the exception of IME composition as [[vue3.#vmodel-ime-tip]]). You can add the `lazy` modifier to instead sync after `change` events:
...
HTML's built-in input types won't always meet your needs. Fortunately, Vue components allow you to build reusable inputs with completely customized behavior. These inputs even work with `v-model`! To learn more, read about [[vue3.guide.components.events#usage-with-v-model]] in the Components guide.
