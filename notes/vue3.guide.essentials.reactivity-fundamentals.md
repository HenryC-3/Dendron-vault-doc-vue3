...
We can create a reactive object or array with the [[vue3.api.reactivity-core#reactive]] function:
...
To wait for the DOM update to complete after a state change, you can use the [[vue3.api.general#nexttick]] global API:
...
It is also possible to explicitly create [[vue3.api.reactivity-advanced#shallowreactive]] where the reactivity is only tracked at the root-level, however they are typically only needed in advanced use cases.
...
To address the limitations of `reactive()`, Vue also provides a [[vue3.api.reactivity-core#ref]] function which allows us to create reactive **"refs"** that can hold any value type:
...
See also: [[vue3.guide.typescript.composition-api#typing-ref]] <sup class="vt-badge ts" />
...
In other words, `ref()` allows us to create a "reference" to any value and pass it around without losing reactivity. This capability is quite important as it is frequently used when extracting logic into [[vue3.guide.reusability.composables]].
...
Ref unwrapping only happens when nested inside a deep reactive object. It does not apply when it is accessed as a property of a [[vue3.api.reactivity-advanced#shallowreactive]].
...
You can learn more about [[vue3.guide.extras.reactivity-transform]] in its dedicated section. Do note that it is currently still experimental and may change before being finalized.
