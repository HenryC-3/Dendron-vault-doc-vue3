...
> If you are new to the concept of virtual DOM and render functions, make sure to read the [[vue3.guide.extras.rendering-mechanism]] chapter first.
...
That's right, this is a valid Vue component! See [[vue3.#functional-components]] for more details on this syntax.
...
- Passing children to components (i.e. slots) [[vue3.#passing-slots]].
...
For other event and key modifiers, the [[vue3.api.render-function#withmodifiers]] helper can be used:
...
If a component is registered by name and cannot be imported directly (for example, globally registered by a library), it can be programmatically resolved by using the [[vue3.api.render-function#resolvecomponent]] helper.
...
In render functions, slots can be accessed from [[vue3.api.component-instance#slots]]:
...
[[vue3.api.built-in-components]] such as `<KeepAlive>`, `<Transition>`, `<TransitionGroup>`, `<Teleport>` and `<Suspense>` must be imported for use in render functions:
...
Custom directives can be applied to a vnode using [[vue3.api.render-function#withdirectives]]:
...
If the directive is registered by name and cannot be imported directly, it can be resolved using the [[vue3.api.render-function#resolvedirective]] helper.
...
The second argument, `context`, contains three properties: `attrs`, `emit`, and `slots`. These are equivalent to the instance properties [[vue3.api.component-instance#attrs]], [[vue3.api.component-instance#emit]], and [[vue3.api.component-instance#slots]] respectively.

</div>

Most of the usual configuration options for components are not available for functional components. However, it is possible to define [[vue3.api.options-state#props]] and [[vue3.api.options-state#emits]] by adding them as properties:
