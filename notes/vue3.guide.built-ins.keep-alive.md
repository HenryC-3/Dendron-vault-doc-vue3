...
In the Component Basics chapter, we introduced the syntax for [[vue3.guide.essentials.component-basics#dynamic-components]], using the `<component>` special element:
...
:::tip
When used in [[vue3.guide.essentials.component-basics#dom-template-parsing-caveats]], it should be referenced as `<keep-alive>`.
:::
...
The match is checked against the component's [[vue3.api.options-misc#name]] option, so components that need to be conditionally cached by `KeepAlive` must explicitly declare a `name` option.
...
We can limit the maximum number of component instances that can be cached via the `max` prop. When `max` is specified, `<KeepAlive>` behaves like an [[vue3.<https:..en.wikipedia.org.wiki.cache_replacement_policies#Least_recently_used_(LRU]]>): if the number of cached instances is about to exceed the specified max count, the least recently accessed cached instance will be destroyed to make room for the new one.
...
A kept-alive component can register lifecycle hooks for these two states using [[vue3.api.composition-api-lifecycle#onactivated]] and [[vue3.api.composition-api-lifecycle#ondeactivated]]:
...
A kept-alive component can register lifecycle hooks for these two states using [[vue3.api.options-lifecycle#activated]] and [[vue3.api.options-lifecycle#deactivated]] hooks:
...
- [[vue3.api.built-in-components#keepalive]]
