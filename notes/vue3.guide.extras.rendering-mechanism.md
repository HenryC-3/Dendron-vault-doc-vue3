...
2. **Mount**: The runtime renderer invokes the render functions, walks the returned virtual DOM tree, and creates actual DOM nodes based on it. This step is performed as a [[vue3.reactivity-in-depth]], so it keeps track of all reactive dependencies that were used.
...
![render pipeline](/assets/render-pipeline-b0cwqwoe1bcl.png)
...
In practice, templates are sufficient for most use cases in applications. Render functions are typically only used in reusable components that need to deal with highly dynamic rendering logic. Render function usage is discussed in more detail in [[vue3.render-function]].
...
Both patch flags and tree flattening also greatly improve Vue's [[vue3.guide.scaling-up.ssr#client-hydration]] performance:
